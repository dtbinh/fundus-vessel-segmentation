...
function [resp] = Azzopardi2015(I, mask, unary, options)
...
%      I -> RGB retinal fundus I, in double, normalized into [0,1]
%      options.symmetric -> a struct defining the configuration parameters of the
%                 symmetric filter:                   
%                   Name                Value
...
%                                       function
%      options.asymmetric -> a struct defining the configuration parameters of the
%                 asymmetric filter:                   
%                   Name                Value
%                   'sigma'             Value of the standard deviation of
%                                       the outer Gaussian funciton in the DoG
...
%
    ...
    % COSFIRE params
    symm_params.inputfilter.DoG.sigmalist = options.symmetric.sigma;
    symm_params.COSFIRE.rholist = 0:2:options.symmetric.len;
    symm_params.COSFIRE.sigma0 = options.symmetric.sigma0 / 6;
    symm_params.COSFIRE.alpha = options.symmetric.alpha / 6;
    % Orientations
    numoriens = 12;
    ...
    % COSFIRE params
    asymm_params.inputfilter.DoG.sigmalist = options.asymmetric.sigma;
    asymm_params.COSFIRE.rholist = 0:2:options.asymmetric.len;
    asymm_params.COSFIRE.sigma0 = options.asymmetric.sigma0 / 6;
    asymm_params.COSFIRE.alpha = options.asymmetric.alpha / 6;
    % Orientations
    numoriens = 24;
    ...
    resp1 = applyCOSFIRE(I, symmfilter);
    resp1 = resp1{1};

    % Apply the asymmetric B-COSFIRE to the input I
    resp2 = applyCOSFIRE(I, asymmfilter);
    resp2 = resp2{1};
