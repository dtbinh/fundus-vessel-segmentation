...
% outputs = 'C:\Users\USUARIO\Dropbox\RetinalImaging\Writing\tmi2015paper\experiments\featureEngineering\Zana';
outputs = 'G:\Dropbox\RetinalImaging\Writing\tmi2015paper\experiments\featureEngineering\Zana';
...
for i = 1 : length(preprocessingMethods)
    ...
    for j = 1 : length(images)
        ...
        for k = 1 : length(parametersSize)
            options.l = parametersSize(k);
            f = imcomplement(Zana2001(I, mask, false, options));
...
ylabel('AUC');
title(strcat('Zana - Parameters on ', {' '}, datasetname));
legend('Without preprocessing', 'With preprocessing', 'Location', 'southeast');
print(strcat(outputs, filesep, 'zana_', datasetname), '-dpng');
print(strcat(outputs, filesep, 'zana_', datasetname), '-dpdf');
