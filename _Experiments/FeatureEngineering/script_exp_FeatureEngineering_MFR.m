...
%outputs = 'C:\Users\USUARIO\Dropbox\RetinalImaging\Writing\tmi2015paper\experiments\featureEngineering\MatchedFilterResponses';
outputs = 'G:\Dropbox\RetinalImaging\Writing\tmi2015paper\experiments\featureEngineering\MatchedFilterResponses';

% folder = 'C:\_tmi_experiments\ARIA\ALL\training';
% windowSize = 47;
% datasetname = 'ARIA';
% fakepad_extension = 50;
% parameters = sqrt(1:2:22);
% 
% folder = 'C:\_tmi_experiments\CHASEDB1\training';
% windowSize = 89;
% datasetname = 'CHASEDB';
% fakepad_extension = 50;
% parameters = sqrt(1:7:70);
% 
folder = 'C:\_tmi_experiments\DRIVE\training-validation';
windowSize = 43;
datasetname = 'DRIVE';
fakepad_extension = 50;
parameters = sqrt(1:2:12);
%
% folder = 'C:\_tmi_experiments\HRF\ALL\training';
% windowSize = 119;
% datasetname = 'HRF';
% fakepad_extension = 100;
% parameters = sqrt(36:10:196);
%
% folder = 'C:\_tmi_experiments\STARE\training';
% windowSize = 43;
% datasetname = 'STARE';
% fakepad_extension = 50;
% parameters = sqrt(1:2:15);
...
for i = 1 : length(preprocessingMethods)
    ...
    for j = 1 : length(images)
        ...
        options.sigmas = parameters;
        feature = MatchedFilterResponses(I, mask, true, options);
        % for each parameter
...
ylabel('AUC');
title(strcat('Matched Filter Responses - Parameters on ', {' '}, datasetname));
legend('Without preprocessing', 'With preprocessing', 'Location', 'southeast');
print(strcat(outputs, filesep, 'mfr_2', datasetname), '-dpng');
print(strcat(outputs, filesep, 'mfr_2', datasetname), '-dpdf');
