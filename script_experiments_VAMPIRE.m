...
% Datasets names
datasetsNames = {'vampire_armd'};
%datasetsNames = {'vampire_ger'};
...
rootResults = 'C:\Users\USUARIO\Dropbox\RetinalImaging\Writing\sipaim2015paper\results';
...
for experiment = 1 : length(datasetsNames)
    ...
    for crfver = 1 : length(crfVersions)
        ...
        [config] = getConfiguration_CrossValidation(datasetsNames{experiment}, datasetsPaths{experiment}, resultsPaths{experiment}, learnC, crfVersions{crfver}, cValue);
        config.preprocessing.preprocess = 0;
        ...
        % Invert all the images
        for i = 1 : length(data.images)
            data.images{i} = imcomplement(data.images{i});
        end
